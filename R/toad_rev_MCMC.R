...
rev_probcor.1<-MCMCglmm(choice ~ sex*trial, random = ~us(1+trial):toad.id, family = "categorical", nitt = 2000000, thin = 1000, prior=prior.test, burnin = 15000, data=toadrev, verbose= T)

saveRDS(rev_probcor.1, file="output/toad_rev_probcormod.1")

#Running probability of making correct choice with interaction

rev_probcor.wo<-MCMCglmm(choice ~ sex+trial, random = ~us(1+trial):toad.id, family = "categorical", nitt = 2000000, thin = 1000, prior=prior.test, burnin = 15000, data=toadrev, verbose= T)
...
toad_lat.rev <- MCMCglmm(log.latency ~ sex*trial, random = ~us(1+trial):toad.id, family = "gaussian", nitt =2000000, thin = 5000, prior=prior.test1, burnin = 10000, data=toadrev, verbose = T)

saveRDS(toad_lat.rev, file="output/toad_rev_lat.1")

#Running log latency model without interaction

toad_lat.rev.wo <- MCMCglmm(log.latency ~ sex+trial, random = ~us(1+trial):toad.id, family = "gaussian", nitt =2000000, thin = 5000, prior=prior.test1, burnin = 10000, data=toadrev, verbose = T)
