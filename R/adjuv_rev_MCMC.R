...
adjuv_rev_lat.1 <- list()
  for(i in 1:3){
  set.seed(chains[i])
    adjuv_rev_lat.1[[i]] <- MCMCglmm(log.latency ~ age*trial, random = ~us(1+trial):toad.id, family = "gaussian", nitt =110000, thin = 100, prior=prior.test1, burnin = 10000, data=adjuv.rev, verbose = T)
  } 
...
adjuv_rev_lat.wo <- list()
  for(i in 1:3){
  set.seed(chains[i])
    adjuv_rev_lat.wo[[i]]<- MCMCglmm(log.latency ~ age+trial, random = ~us(1+trial):toad.id, family = "gaussian", nitt =110000, thin = 100, prior=prior.test1, burnin = 10000, data=adjuv.rev, verbose = T)
  }
